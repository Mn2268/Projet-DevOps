...
async function loadCandidates() {
  try {
    const res = await fetch('../core/candidate-handler.php?action=get_all');
...
async function prefillPositionForElection(electionId) {
  try {
    const res = await fetch(`../core/api.php?action=getPositionByElection&id_election=${encodeURIComponent(electionId)}`);
...
async function handleCreate(e) {
  ...
  try {
    ...
    const res = await fetch('../core/candidate-handler.php', {
      method: 'POST',
      body: formData,
    });
    const data = await res.json();
...
async function handleDeleteCandidate(id) {
  ...
  try {
    ...
    const res = await fetch('../core/candidate-handler.php', { method: 'POST', body: fd });
    const data = await res.json();
    if (data.success) {
      await loadCandidates();
      const electionId = getQueryParam('id_election');
