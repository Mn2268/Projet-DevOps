...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchElections() {
    try {
      const response = await fetch('core/api.php?action=getActiveElections');
  ...
  async function fetchPositionsByElection(electionId) {
    try {
      const response = await fetch(`core/api.php?action=getPositionByElection&id_election=${electionId}`);
  ...
  async function fetchCandidatesByPosition(positionId) {
    try {
      const response = await fetch(`core/api.php?action=getCandidatesByPosition&id_position=${positionId}`);
  ...
  async function submitVoteToHedera(candidateId, positionId) {
    try {
      ...
      const response = await fetch('core/hedera-bridge.php', {
        method: 'POST',
        body: formData
      });
